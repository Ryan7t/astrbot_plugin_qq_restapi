...
├── adapters/            ← 平台适配器（WS / Webhook 两种模式）
│   ├── qq_restapi_adapter.py         (WebSocket Gateway)
│   └── qq_restapi_webhook_adapter.py (HTTP Webhook)
├── runtime/             ← 运行时核心
│   ├── context.py          全局上下文单例（Context/Config/DB）
│   ├── sender.py           QQ REST API 消息发送（文本/Markdown/图片/媒体）
│   ├── qq_restapi_event.py AstrMessageEvent 实现（reply/reply_markdown 等）
│   ├── message_parser.py   QQ 事件类型常量 + 事件解析
│   ├── auto_events.py      自动事件处理（入群/好友/表态等）
│   ├── bot_api_client.py   后端 Bot API 客户端（HMAC-SHA256 签名）
│   ├── httpx_pool.py       全局 httpx 连接池
│   ├── token_manager.py    QQ API Token 管理
│   ├── template_registry.py / template_store.py  Markdown 模板系统
│   └── last_message_cache.py
├── commands/            ← 业务指令实现
├── db/                  ← 数据库层（SQLModel + SQLite）
...
**凭证获取**：`plugin._get_app_credentials()` 从已注册的 QQ 平台实例提取 appid/secret。
...
所有 HTTP 请求复用 `runtime/httpx_pool.get_async_client()` 全局连接池，不要创建新的 httpx 客户端实例。

### 适配器热重载

适配器在注册前会清理旧注册，避免插件重载时重复注册。修改适配器时需保留此机制。

### 数据库

SQLModel + SQLite（WAL 模式）。表定义在 `db/models.py`，数据操作在 `db/repository.py`，业务逻辑在 `db/service.py`。`QQRestAPIDatabase` 在 `initialize()` 时自动建表。
