...
本项目仅负责 AstrBot 的 QQ REST API 插件适配器。**严禁修改 AstrBot 框架本体的任何代码或文件**；所有代码、配置、文档及资源变更，都必须严格限制在当前插件项目目录 `AstrBot/data/plugins/qq_restapi/` 内。不得为了实现插件功能而直接修改该目录之外的框架模块、公共组件、启动逻辑、依赖管理文件或其他插件，也不得通过复制、覆盖、补丁脚本等方式间接改写框架代码。

如在分析或实现过程中发现需求依赖 AstrBot 框架能力，且仅修改本插件无法安全、完整地实现，必须立即停止相关修改，不得擅自扩大变更范围或先行修改框架。此时应向用户明确说明：需要修改的框架位置、必须修改的原因、预期影响与风险，以及是否存在插件侧替代方案，并与用户进行充分、反复确认。只有在用户明确调整任务范围并授权框架级变更后，才能另行处理；在此之前，始终以“框架代码只读、插件目录可修改”为不可突破的边界。
